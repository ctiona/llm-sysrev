"""
Pn-CR Framework: multiple expert-background agent iterative screening framework with conflict resolution.

Architecture
------------
  Multiple agents (≥ 2) with distinct expert backgrounds (defined in syscontext.py)
  perform title/abstract screening independently, two per paper (chosen at random).
  Where the two agents disagree or either votes UNSURE, a multi-turn conversation
  is performed until consensus is reached.  The same pattern is applied at
  full-text level after a validity check.

  Pn-CR differs from B2-CR only in that it incorporates multiple agents with
  unique researcher background agents rather than one generic "Basic" agent.
...
# ---------------------------------------------------------------------------
# Title / abstract screening: two random persona agents per entry
# ---------------------------------------------------------------------------
...
def screen_titles_parallel(
    ...
    """
    For each entry draw two persona agents at random, screen independently,
    and store decisions as entry[f"{agent_id}_decision"].
    """
    agents = cfg["agents"]
    gen_kwargs = cfg.get("generation_kwargs", {})

    def process(entry: dict) -> None:
        for agent_id in random.sample(agents, 2):
            agent = Agent(
                model=cfg["model"],
                background=str(agent_id),
                task="T&A_Screening",
            ...
            entry[f"{agent_id}_decision"] = decision
...
def resolve_conflicts(
    ...
    run a multi-turn conversation between the two persona agents until they reach
    consensus.  Each agent retains its persona background throughout the dialogue.

    Parameters
    ----------
    decision_prefix : prefix of the two decision fields (e.g. "" for T&A,
                      "Pn-CR_fulltext_" for full-text)
    ...
    def process_one(pmid: str, entry: dict) -> str:
        ...
        for e in entries_xml["root"]["entry"]:
            if str(e.get("id")) == pmid:
                e["Pn-CR_fulltext_validity"] = decision
                break
...
# ---------------------------------------------------------------------------
# Full-text initial screening: two random persona agents per paper
# ---------------------------------------------------------------------------

def screen_fulltexts_initial(
    short_texts: dict, entries_xml: dict, cfg: dict
) -> None:
    """Two-agent full-text screening; decisions stored as Pn-CR_fulltext_{agent_id}_decision."""
    agents = cfg["agents"]
    gen_kwargs = cfg.get("generation_kwargs", {})
    ...
    def process_one(pmid: str, data: dict) -> None:
        ...
        if entry is None:
            return
        if entry.get("Pn-CR_fulltext_validity", "").split(" - ")[0] == "INVALID":
            for agent_id in random.sample(agents, 2):
                entry[f"Pn-CR_fulltext_{agent_id}_decision"] = "EXCLUDE - Invalid paper"
            return
        ...
        for agent_id in random.sample(agents, 2):
            ...
            _, decision = agent.reply_to(prompt)
            entry[f"Pn-CR_fulltext_{agent_id}_decision"] = unmark(decision)
...
def run_one_cycle(cycle_idx: int, cfg: dict) -> dict:
    ...
    ta_transcripts = resolve_conflicts(
        entries, decision_prefix="", consensus_key="Pn-CR_ta_consensus",
        short_texts=None, cfg=cfg,
    )
    ta_counts = {}
    for e in entries["root"]["entry"]:
        k = e.get("Pn-CR_ta_consensus", "")
        if k:
    ...
    with open(transcript_dir / f"ta_transcript_Pn-CR_run{cycle_idx + 1}.json", "w", encoding="utf-8") as f:
    ...
    included = [
        ...
        for e in entries["root"]["entry"]
        if e.get("Pn-CR_ta_consensus", "").upper() == "INCLUDE"
    ]
    ...
    ft_transcripts = resolve_conflicts(
        entries, decision_prefix="Pn-CR_fulltext_", consensus_key="Pn-CR_fulltext_consensus",
        short_texts=short_texts, cfg=cfg,
    )
    ft_dir = Path(cfg.get("transcript_dir", "transcripts")) / "fulltext"
    ft_dir.mkdir(parents=True, exist_ok=True)
    with open(ft_dir / f"ft_transcript_Pn-CR_run{cycle_idx + 1}.json", "w", encoding="utf-8") as f:
        json.dump(ft_transcripts, f, indent=2, ensure_ascii=False)

    final_counts = {"INCLUDE": 0, "EXCLUDE": 0, "UNCLEAR": 0}
    for e in entries["root"]["entry"]:
        key = e.get("Pn-CR_fulltext_consensus", "").split(" - ")[0].upper()
        if key:
    ...
    export_included(
        entries,
        consensus_key="Pn-CR_fulltext_consensus",
...
def main():
    parser = argparse.ArgumentParser(description="Pn-CR: persona-agent pool systematic review framework")
